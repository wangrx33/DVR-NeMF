...
from run_nerf_helpers_biflow import *
...
def render_path(D, H, W, input_slices, input_times, chunk, render_kwargs, gt=None, savedir=None):
    ...
    volumes_img = []
    t = time.time()
    coords_slice_0, coords_time_0 = get_coords(D, H, W, input_slices[0], input_times[0])
...
def render_rays(pts_batch_0,
    ...
    region_0 = torch.linspace(0., 1., steps=N_samples)
    if not lindisp:
...
def config_parser():
    ...
    parser.add_argument("--expname", type=str, default='2d-3d_0518_biflow_dxloss',
...
def train():
    ...
    if args.render_only:
        ...
        with torch.no_grad():
            ...
            recon_volumes_img_0, recon_volumes_img = render_path(D, H, W, input_slices_img, input_times, args.chunk, render_kwargs_test)
            # time t_0
            
            # time t_i
            psnr, ssim = test_metrics(recon_volumes_img, input_volumes_img.cpu().numpy())
    ...
    for i in range(start, N_iters):
        ...
        for epoch in range(num_epoch):
            ...
            temporal_loss = img2mae((volume_img - volume_img_0), (target_s_img - target_s_img_0)) 
