...
from run_nerf_helpers_biflow_new import *
...
def batchify(fn, chunk):
    ...
    if chunk is None:
        return fn
    def ret(inputs_embed_0, inputs_pts_0, inputs_embed, inputs_pts):  
        volume_0 = []
        volume = []
        for i in range(0, inputs_embed.shape[0], chunk):
            res_0, res = fn(inputs_embed_0[i:i+chunk], inputs_pts_0[i:i+chunk], inputs_embed[i:i+chunk], inputs_pts[i:i+chunk])
            volume_0.append(res_0)
            volume.append(res)
        return torch.cat(volume_0, 0), torch.cat(volume, 0)
...
def run_network(inputs_0, time_info_0, inputs, time_info, fn, embed_fn, embedtime_fn, netchunk=1024*64):
    '''
    ...
    '''
    # time t_0
    inputs_flat_0 = torch.reshape(inputs_0[...,:4], [-1, 4]) # 1 024*64, 1
    embedded_spatial_0 = embed_fn(inputs_flat_0)    # 1024*64, 21

    if time_info_0 is not None:
        inputs_time_0 = time_info_0[:,None].expand(inputs_0[...,4:].shape)
        inputs_time_flat_0 = torch.reshape(inputs_time_0, [-1, inputs_time_0.shape[-1]])
        embedded_time_0 = embedtime_fn(inputs_time_flat_0)  # 1024*64, 64
        embedded_temporal_0 = torch.zeros(size=(embedded_time_0.shape[0], embedded_time_0.shape[1]+1))
        embedded_temporal_0[:,:-1] = embedded_time_0
        embedded_temporal_0[:,-1] = inputs_time_flat_0[:,-1]
        embedded_0 = torch.cat([embedded_spatial_0, embedded_temporal_0], -1)     # 1024*64, 85
    ...
    if time_info is not None:
        ...
        embedded_time = embedtime_fn(inputs_time_flat)  # 1024*64, 64
        embedded_temporal = torch.zeros(size=(embedded_time.shape[0], embedded_time.shape[1]+1))
        embedded_temporal[:,:-1] = embedded_time
        embedded_temporal[:,-1] = inputs_time_flat[:,-1]
        embedded = torch.cat([embedded_spatial, embedded_temporal], -1)     # 1024*64, 85

    outputs_flat_0, outputs_flat = batchify(fn, netchunk)(embedded_0, inputs_flat_0, embedded, inputs_flat) # 1024*64, 1
    outputs_0 = torch.reshape(outputs_flat_0, list(inputs_0.shape[:-1]) + [outputs_flat_0.shape[-1]])   # 1024, 64, 1
    outputs = torch.reshape(outputs_flat, list(inputs.shape[:-1]) + [outputs_flat.shape[-1]])   # 1024, 64, 1
    return outputs_0, outputs

def batchify_rays(pts_flat_0, pts_flat, chunk=1, **kwargs):
    all_ret = {}
    for i in range(0, pts_flat.shape[0], chunk):
        ret = render_rays(pts_flat_0[i:i+chunk], pts_flat[i:i+chunk], **kwargs)
        for k in ret:
            if k not in all_ret:
                all_ret[k] = []
...
def render(D, H, W, s_t_0, s_t, chunk=None, neigh=10, test=False, **kwargs):
    '''
    s_t: 2, N_rand, 1
    '''
    sh = (D, H, W)
    # time t_0
    pts_slice_0, pts_time_0 = s_t_0
    neigh_0 = neigh * torch.ones_like(pts_time_0)
    pts_slice_0 = torch.reshape(pts_slice_0, [-1,4]).float()
    pts_time_0 = torch.reshape(pts_time_0, [-1,4]).float()
    pts_neigh_0 = torch.reshape(neigh_0, [-1,4]).float()
    pts_flat_0 = torch.cat([pts_slice_0, pts_neigh_0[...,0:1], pts_time_0], -1)  # N_rand, 3
    ...
    all_ret = batchify_rays(pts_flat_0, pts_flat, chunk, **kwargs)
    if test == True:
        for k in all_ret:
            all_ret[k] = torch.reshape(all_ret[k], sh)
    k_extract = ['volume_img_0', 'volume_img']
...
def render_path(D, H, W, ref_slice, ref_time, input_slices, input_times, chunk, render_kwargs, gt=None, savedir=None):
    '''
    input_slices: N,H,W
    input_times: N,1
    '''
    ...
    volumes_img = []
    t = time.time()
    coords_slice_0, coords_time_0 = get_coords(D, H, W, ref_slice, ref_time)
    pts_slice_0 = torch.reshape(coords_slice_0, [-1,1])
    pts_time_0 = torch.reshape(coords_time_0, [-1,1])
    test_pts_0 = torch.stack([pts_slice_0, pts_time_0], 0)
    for i, (i_slice, i_time) in enumerate(tqdm(zip(input_slices[1:], input_times[1:]))):
        # slice: H,W / time: 1
        ...
        t = time.time()
        volume_img_0, volume_img = render(D, H, W, s_t_0=test_pts_0, s_t=test_pts, chunk=chunk, test=True, **render_kwargs)
        
        # volume_img_real = torch.abs(torch.fft.ifftn(volume_k))
        volumes_img_0.append(volume_img_0.cpu().numpy())
        volumes_img.append(volume_img.cpu().numpy())

    volumes_img_0 = np.stack(volumes_img_0, 0)
    volumes_img = np.stack(volumes_img, 0)
    return volumes_img_0, volumes_img
...
def create_nerf(D,H,W,args):
    ...
    model = D_NeRF(D=args.netdepth, W=[128, 256, 256, 512, 256, 256, 256, 256],
                input_ch=input_ch, output_ch=output_ch, skips=skips,
                spatial_dim=input_ch_s, temporal_dim=input_ch_time, hidden_dim=256, num_layers=8, embed_fn=embed_fn).to(device)
    ...
    network_query_fn = lambda inputs_0, time_info_0, inputs, time_info, network_fn : run_network(inputs_0, time_info_0, 
                                                                                                inputs, time_info, 
                                                                                                network_fn, 
                                                                                                embed_fn=embed_fn,
...
def raw2outputs(raw_0, raw):
    ...
    '''
    volume_img_0 = torch.sum(raw_0, 1)   # 1024, 1   k-space
    # volume_k_0 = torch.abs(torch.fft.ifftn(volume_img_0)) # 1024, 1 image

    volume_img = torch.sum(raw, 1)   # 1024, 1   k-space
    # volume_k = torch.abs(torch.fft.ifftn(volume_img)) # 1024, 1 image
    return volume_img_0, volume_img

def render_rays(pts_batch_0,
                pts_batch, # N_chunk, 3
                network_fn, 
                network_query_fn, 
                retraw=False,
    ...
    # time t_0
    pts_slice_0, pts_neigh_0, pts_time_0 = pts_batch_0[:,0:4], pts_batch_0[:,4:5], pts_batch_0[:,5:]
    pts_slice_0 = torch.reshape(pts_slice_0, [-1,4]).float()
    pts_time_0 = torch.reshape(pts_time_0, [-1,4]).float()
    pts_neigh_0 = torch.reshape(pts_neigh_0, [-1,1]).float()
    region_0 = torch.linspace(0., 100., steps=N_samples)
    if not lindisp:
        region_0 = pts_neigh_0 * region_0

    #对region进行随机采样，
    pts_0 = torch.cat([pts_slice_0[...,None,:] * region_0[...,:,None], pts_time_0[...,None,:] * region_0[...,:,None]], -1) 
    ...
    raw_0, raw = network_query_fn(pts_0, pts_time_0, pts, pts_time, network_fn)
    volume_img_0, volume_img = raw2outputs(raw_0, raw)
    
    ret = {'volume_img_0': volume_img_0, 'volume_img': volume_img}
    if retraw:
        ret['raw_0'] = raw_0
        ret['raw'] = raw
    for k in ret:
...
def config_parser():
    ...
    parser.add_argument("--expname", type=str, default='2d-3d_0522_split_150',
    ...
    parser.add_argument("--N_rand", type=int, default=1024*9, 
                        help='batch size (number of random rays per gradient step)')
    parser.add_argument("--lrate", type=float, default=1e-3, 
                        help='learning rate')
    parser.add_argument("--lrate_decay", type=int, default=5000, 
...
def train():
    ...
    if args.dataset_type == 'mri':
        ...
        trainset, testset = load_mri_data(args.datadir)
        volumes_img, volumes_k, slices_img, slices_k, times = trainset
        testvolumes_img, testvolumes_k, testslices_img, testslices_k, testtimes = testset
    ...
    input_test_slices_img = torch.tensor(testslices_img, dtype=torch.float64).to(device)
    ...
    if args.render_only:
        ...
        with torch.no_grad():
            ...
            recon_volumes_img_0, recon_volumes_img = render_path(D, H, W, input_slices_img[0], input_times[0], input_test_slices_img, input_test_times, args.chunk, render_kwargs_test)
            # time t_0
    ...
    for i in range(start, N_iters):
        # data_loader   B=1
        # input at time t_0
        i_volume_k_0 = input_volumes_k[0]   # D, H, W
        i_volume_img_0 = input_volumes_img[0]   # D, H, W
        i_slice_k_0 = input_slices_k[0]     # H, W
        i_slice_img_0 = input_slices_img[0]     # H, W
        i_time_0 = input_times[0]       # 1
        ...
        for epoch in range(num_epoch):
            ...
            sample_p[diff>0.02] = 10
            # sample_p = torch.tile(sample_p, 10)
            ...
            volume_img_0, volume_img = render(D, H, W, batch_pts_0, batch_pts, chunk=args.chunk, **render_kwargs_train)
            optimizer.zero_grad()
        ...
        if i%args.i_print==0:
            ...
            logging.info(f"[TRAIN] Duration:{currenttime} Iter: {i} Loss: {loss.item()} PSNR: {psnr.item()} Pred: {volume_img.detach().cpu().numpy().max(), volume_img.detach().cpu().numpy().mean()} GT: {target_s_img.mean()}")
