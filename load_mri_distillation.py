...
# 先把所有训练数据读到内存里
def load_mri_data(basedir, patient_id):
    '''
    ...
    all_patients = []
    patientslist = os.listdir(basedir)
    patientslist.sort()
    patientslist = patientslist[patient_id-1:patient_id]
    print(patientslist)
    # patientslist.sort()
    ...
    all_data = all_data_whole[:, :, 20:80, 20:80, 20:80]  # crop 128*128 size due to memory limitation-
    # all_data = all_data_whole  # crop 128*128 size due to memory limitation-
    ...
    # volumes_img = np.zeros_like(all_data).astype(np.float64)   # (phase_num, slice_num, height, width)
    # for i in range(volumes_img.shape[0]):
    #     volumes_img[i] = normalize(all_data[i])
    # volumes_k = np.zeros_like(all_data).astype(np.complex128)    # (phase_num, slice_num, height, width)
    # for i in range(volumes_k.shape[0]):
    #     volumes_k[i] = (np.fft.fftn(all_data[i])) / np.sqrt((1e+10)*2*np.pi)

    slices_img_a = volumes_img[:,int(slice_num/2),:,:]    # 这里对a/s/c哪个视角作为输入没有进行分析测试
    slices_img_c = volumes_img[:,:,int(slice_num/2),:]
    slices_img_s = volumes_img[:,:,:,int(slice_num/2)]

    slices_imgs = np.stack((slices_img_a, slices_img_c, slices_img_s), -1)  # phase_num, h, w, 3
    ...
    trainset =[volumes_img[:split], volumes_k[:split], slices_imgs[:split], slices_k[:split], times[:split]]
    testset =[volumes_img[split:], volumes_k[split:], slices_imgs[split:], slices_k[split:], times[split:]]

    # return volumes_img, volumes_k, slices_img, slices_k, times
    return trainset, testset


def load_mri_data_singleview(basedir):
    '''
    数据集是由N位病人，每位病人的M个分相数据，每个分相数据为一个3D Volume(D,H,W)组成，即数据总维度为(N,M,D,H,W)
    如果我们的模型是patient-specific的话，那就令N=1
    '''
    ...
    # slices_imgs = np.stack((slices_img_a, slices_img_c, slices_img_s), -1)  # phase_num, h, w, 3
    slices_imgs = np.stack((slices_img_a, slices_img_a, slices_img_a), -1)  # phase_num, h, w, 3
