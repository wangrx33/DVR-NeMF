...
def load_mri_data(basedir):
    ...
    all_data_whole = np.stack(all_patients[0:1], 0)    # patients_num, phase_num, slice, height, width
    all_data = all_data_whole#[:, :, 64:192, 64:192, 64:192]  # crop 128*128 size due to memory limitation-
    all_data = transform.resize(all_data, (1,2,64,64,64))
    # all_data[all_data>0.2] = 0.4
    ...
    slices_img = volumes_img[:,int(slice_num/2),:,:]
    slices_k = volumes_k[:,int(slice_num/2),:,:]
    times = np.arange(0,1,0.5)    # (phase_num,)
