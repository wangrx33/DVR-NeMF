...
writer = SummaryWriter('./training_logs/rand_sampling')
...
def render(D, H, W, s_t, chunk=None, neigh=5, test=False, **kwargs):
    '''
    s_t: 2, N_rand, 1
    '''
    sh = (D, H, W)
...
def create_nerf(D,H,W,args):
    ...
    model = NeRF(D=args.netdepth, W=[128, 256, 256, 512, 256, 256, 256, 256],
                ...
                spatial_dim=input_ch_s, temporal_dim=input_ch_time, hidden_dim=256, num_layers=8, region=args.N_samples).to(device)
...
def config_parser():
    ...
    parser.add_argument("--expname", type=str, default='12_ablation_rand_sampling',
                        help='experiment name')
    parser.add_argument("--patient_id", type=int, default=12,
                        help='patient id')
    ...
    parser.add_argument("--N_rand", type=int, default=1024*20, 
                        help='batch size (number of random rays per gradient step)')
    parser.add_argument("--lrate", type=float, default=1e-3, 
                        help='learning rate')
    parser.add_argument("--lrate_decay", type=int, default=10000, 
...
def train():
    ...
    for i in range(start, N_iters):
        ...
        for epoch in range(num_epoch):
            ...
            mask[attention>(attention.median()/2*3)] = 10
            ...
            # img_loss_0 = img2mse(torch.squeeze(volume_img_0), target_s_img_0) + 10 * sw_loss(torch.squeeze(volume_img_0), target_s_img_0) #+ ssim_loss(torch.squeeze(volume_img_0), target_s_img_0) 
            # img_loss = img2mse(torch.squeeze(volume_img), target_s_img) + 10 * sw_loss(torch.squeeze(volume_img), target_s_img) #+ ssim_loss(torch.squeeze(volume_img), target_s_img)
        ...
        if i%args.i_print==0:
            ...
            writer.add_scalar('Spatial_Loss', img_loss_0.item() + img_loss.item(), i)
            writer.add_scalar('Temporal_Loss', temporal_loss.item(), i)
