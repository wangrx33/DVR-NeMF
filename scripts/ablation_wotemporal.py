...
writer = SummaryWriter('./training_logs/wo_temporal')
...
def config_parser():
    ...
    parser.add_argument("--expname", type=str, default='12_ablation_wo_temporal',
...
def train():
    ...
    if args.dataset_type == 'mri':
        ...
        # trainset, testset = load_mri_data_singleview(args.datadir, args.patient_id)
    ...
    for i in range(start, N_iters):
        ...
        for epoch in range(num_epoch):
            ...
            psnr = mse2psnr(img2mse(torch.squeeze(volume_img_0), target_s_img_0)) #+ mse2psnr(k_loss)
        ...
        if i%args.i_print==0:
            ...
            logger.info(f"[TRAIN] Duration:{currenttime} Iter: {i} Loss: {loss.item()} PSNR: {psnr.item()} Pred: {volume_img_0.detach().cpu().numpy().max(), volume_img_0.detach().cpu().numpy().mean()} GT: {target_s_img_0.mean()}")
