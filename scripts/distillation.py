...
log_path = './training_logs/w_dis_4_64'
...
def run_network(inputs, time_info, fn, embed_fn, embedtime_fn, spatial_dim, temporal_dim, test=False):
    ...
    if test:
        ...
        render = radiance_field.view(-1, 2)   # B, 27
...
def create_teacher_student_nerf(args):
    ...
    skips_s = [2]
    student = NeRF_2(D=args.netdepth, W=[128, 256, 256, 512, 256, 256, 256, 256],
                input_ch=input_ch, output_ch=output_ch, skips=skips_s,
                spatial_dim=input_ch_s, temporal_dim=input_ch_time, hidden_dim=64, num_layers=4, region=args.N_samples_s).to(device)
...
def config_parser():
    ...
    parser.add_argument("--expname", type=str, default='12_lijinkai_w_dis_4_64',
    ...
    parser.add_argument("--pretrain", type=str, default='./logs/12_lijinkai_wo_dis_8_256/02000  0.tar', 
                        help='input data directory')
    ...
    # rendering options
    parser.add_argument("--N_samples_t", type=int, default=2, 
                        help='number of coarse samples per ray')
    parser.add_argument("--N_samples_s", type=int, default=2, 
                        help='number of coarse samples per ray')
...
def train():
    ...
    for i in range(start, N_iters):
        ...
        for epoch in range(num_epoch):
            ...
            out_loss = img2mse(output_student, output_teacher) + 10*sw_loss(output_student, output_teacher)
