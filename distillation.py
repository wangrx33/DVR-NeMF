...
def run_network(inputs, time_info, fn, embed_fn, embedtime_fn, spatial_dim, temporal_dim, test=False):
    ...
    if test:
        ...
        render = radiance_field.view(-1, 27)   # B, 27
...
def render(D, H, W, s_t, chunk=None, neigh=3, test=False, render_only=False, **kwargs):
    '''
    s_t: 2, N_rand, 1
    '''
    sh = (D, H, W)
...
def config_parser():
    ...
    parser.add_argument("--expname", type=str, default='2d-3d_0605_distillation_3_4*128',
                        help='experiment name')
    parser.add_argument("--basedir", type=str, default='./logs/', 
                        help='where to store ckpts and logs')
    parser.add_argument("--datadir", type=str, default='./dataset/mri/my3d_t', 
                        help='input data directory')
    parser.add_argument("--pretrain", type=str, default='./logs/2d-3d_0530_wrx/016999.tar', 
                        help='input data directory')
    ...
    # rendering options
    parser.add_argument("--N_samples_t", type=int, default=3*3*3, 
                        help='number of coarse samples per ray')
    parser.add_argument("--N_samples_s", type=int, default=3, 
                        help='number of coarse samples per ray')
...
def train():
    ...
    if args.render_only:
        ...
        with torch.no_grad():
            ...
            recon_volumes_img = render_path(D, H, W, new_input_test_slices_imgs, new_input_test_times, args.chunk, render_kwargs_test)
            # time t_0
    ...
    for i in range(start, N_iters):
        ...
        for epoch in range(num_epoch):
            ...
            # training
            optimizer.zero_grad()
            feat_loss = img2mse(feature_student, feature_teacher) #+ img2mse(x_student, x_teacher) + img2mse(t_student, t_teacher)
            out_loss = img2mse(output_student, output_teacher) #+ 10*sw_loss(output_student, target_s_img)
            loss = out_loss + 10*feat_loss #+ 100*(torch.abs((output_student.max()-output_student.min())-(output_teacher.max()-output_teacher.min())))
